...
Players can open a session log and review a chronological history of meaningful events — character creations, stat changes, and battle outcomes — giving everyone a shared record of what happened during the session.
...
Players can review a chronological history of character events and battle outcomes in a dedicated log view, backed by a dedicated log service that captures SNS events from character and battle services.

### Story 6.1: `log-service` Backend `[TODO]`

As a developer,
I want a `log-service` Lambda microservice scaffolded from `room-notifications-service` + `character-service` hybrid,
So that session events are captured from SNS and served via HTTP for the log view.

**Acceptance Criteria:**

**Given** the service is scaffolded with two Lambda functions
**When** `logWriter` starts
**Then** if `LOG_TOPIC_ARN` is absent, the Lambda fails with an error — missing configuration is not a recoverable state for the subscriber

**Given** `logWriter` receives an SNS event
**When** the event is processed
**Then** it writes a `LogEvent` document to MongoDB with the correct `roomId`, `type`, `summary`, and `createdAt`
**And** it handles `character_created`, `character_updated`, `character_deleted`, `battle_started`, `battle_concluded`, and `battle_discarded` — `battle_updated` is NOT logged

**Given** `logReader` receives a `GET /logs?roomId=X&limit=50&before=<_id>` request
**When** the request is processed
**Then** it returns paginated log entries filtered strictly by `roomId` (cursor-based via MongoDB `_id`)
**And** the compound index `{ roomId: 1, _id: -1 }` is in place
**And** test coverage meets the 70% floor targeting: SNS handler writes correct document; HTTP reader returns filtered roomId results

### Story 6.2: `character-service` SNS Publisher Extension `[TODO]`

As a developer,
I want `character-service` to also publish to `LOG_TOPIC_ARN` alongside the existing notifications topic,
So that character events are captured in the session event log.

**Acceptance Criteria:**

**Given** a character is created, updated, or deleted
**When** `character-service` processes the event
**Then** it publishes to both `NOTIFICATIONS_TOPIC_ARN` and `LOG_TOPIC_ARN` via `Promise.allSettled`
**And** a missing `LOG_TOPIC_ARN` env var logs a startup warning but does not crash the service
**And** the event payload includes sufficient display context for `log-service` to build a summary string without outbound HTTP calls

### Story 6.3: Wire `LOG_TOPIC_ARN` into `battle-service` `[TODO]`

As a developer,
I want `battle-service` to publish to `LOG_TOPIC_ARN` (replacing the mock from Story 5.1),
So that battle lifecycle events are captured in the session event log.
...
**And** the event payload includes sufficient display context (battle name, result, character names) for `log-service` to build a summary without outbound HTTP calls

### Story 6.4: Log API Client & Hook `[TODO]`

As a developer,
I want `logs.ts` API module and `useRoomLogs` hook,
So that the frontend can fetch paginated log entries using the established pattern.

**Acceptance Criteria:**

**Given** the hook is implemented
**When** the Log View mounts
**Then** `useRoomLogs` fetches the first page of log entries for the room
**And** scrolling to the bottom of the list loads the next page via cursor
**And** TanStack Query key shape is `['logs', roomId]`

### Story 6.5: Log View Screen `[TODO]`

As a player,
I want to open a log screen and review a chronological history of session events,
So that I and my group have a shared record of what happened during the session.

**Acceptance Criteria:**

**Given** I am in the Room View
**When** I navigate to the Log View
