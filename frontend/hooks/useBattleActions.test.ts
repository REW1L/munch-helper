...
import { concludeBattle, discardBattle, patchBattle, startBattle } from '@/api/battles';
...
const mockDiscardBattle = vi.mocked(discardBattle);
...
describe('useBattleActions', () => {
  ...
  it('discards a battle and invalidates the room battle query', async () => {
    const invalidateSpy = vi.spyOn(queryClient, 'invalidateQueries');
    mockDiscardBattle.mockResolvedValue({
      id: 'battle-1',
      roomId: 'room-1',
      name: 'Battle',
      status: 'discarded',
    ...
    await act(async () => {
      await result.current.discard('battle-1');
    });

    expect(mockDiscardBattle).toHaveBeenCalledWith('battle-1');
    expect(invalidateSpy).toHaveBeenCalledWith({ queryKey: ['battle', 'room-1'] });
  });

  it('surfaces discard conflicts through errorMessage', async () => {
    mockDiscardBattle.mockRejectedValueOnce(new ApiError('Battle is not active', 409, { message: 'Battle is not active' }));
    const { result } = renderHook(() => useBattleActions('room-1'), { wrapper });

    await act(async () => {
      await expect(result.current.discard('battle-1')).rejects.toThrow('Battle is not active');
    });
