...
UX-DR3: Implement room-code copy affordance in Room View `Stack.Screen` header options (same visual layer as the room title). Display room code in `accent` color with an inline copy-to-clipboard button (`expo-clipboard`). Copy interaction: inline "Copy" → "Copied ✓" state change for 1500ms, then reset. Accessible: `accessibilityLabel="Copy room code [code]"`, `accessibilityRole="button"`.
...
UX-DR9: Create `useRealtimeFlash(color: string)` hook — border color interpolation from `surfaceWarm` → `color` → `surfaceWarm` over 700ms via `react-native-reanimated` (or equivalent React Native animation primitive). Border only (not background) to avoid layout repaints. Supports concurrent multi-card updates without visual conflict. Trigger on realtime room character updates, while suppressing likely websocket echoes from the same client. Reduced motion: skip interpolation, apply border color immediately and restore `surfaceWarm` after 700ms.
...
UX-DR15: Optimistic updates with error revert — stat value updates immediately in UI on local tap (before server confirmation). `useRealtimeFlash` provides supplementary room-awareness feedback when a server-driven room update is surfaced and not suppressed as a likely local websocket echo. On server error: revert value quietly with a brief border flash in `danger` color. No toast for errors on stat changes.
UX-DR16: Implement reduced motion support — check `useReducedMotion()` from `react-native-reanimated` app-wide. When enabled: `useRealtimeFlash` skips border interpolation (apply immediately, restore `surfaceWarm` after 700ms); `QuickEditSheet` snaps directly to open/closed position instead of spring animation.
