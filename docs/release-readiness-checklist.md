...
Stories 7.7 (Supportability Signals) and 7.8 (Diagnostic Validation Matrix) are landed. The structured `support.failure` signal (taxonomy: `room`, `character`, `battle`, `log`, `session_continuity`) is the canonical observability surface that backs each failure-signature line in this section: a release reviewer must verify the signal is *absent* in the candidate build per Story 7.7's instrumentation, and the diagnostic validation matrix run for the same candidate (see the Release Evidence Record below) is the proof that the signal is observable at all. See `docs/release-support-reference.md` for the signal contract and `docs/release/diagnostic-validation-matrix.md` for the validation procedure.
...
- [ ] [All] **Release blocker — Diagnostic Validation Matrix:** the run artifact under `docs/release/runs/diagnostic-validation-<release-candidate-id>-<YYYY-MM-DD>.md` exists for the *same* `<release-candidate-id>` recorded in the Release Identity block, has `status: finalized` in its header, every scenario row is `pass` or `waived` (no `fail`, no blanks), and every active waiver names a decision-maker, follow-up commitment, and `expires_at`. Tick this box only when all four conditions hold; if any condition is missing, the release is `No-Go` until either the matrix run is corrected or every blocking failure is explicitly waived per the matrix's gate criteria. Link the run artifact path inline.
