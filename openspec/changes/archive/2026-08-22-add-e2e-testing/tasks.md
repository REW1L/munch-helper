...
- [x] 8.1 Reduce the GitHub Actions E2E workflow to the web `e2e-web` job, retaining its 120-minute timeout and teardown
...
- [x] 8.7 Run the iOS, Android, and web E2E suites locally after the native runner fix
- [x] 8.8 Use Maestro's default headless Chromium viewport; forcing `1920x1080` made the CI suite exceed its timeout
...
- [x] 8.10 Preserve the shared `id` selectors in web-flow copies; Maestro resolves React Native Web `data-testid` through them, avoiding the CSS-selector performance regression
