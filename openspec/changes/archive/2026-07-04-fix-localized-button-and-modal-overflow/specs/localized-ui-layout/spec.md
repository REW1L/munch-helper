...
Button labels SHALL render on a single line across all supported languages. When a localized label is wider than its button, the label MUST shrink to fit rather than wrap to a second line, down to a defined minimum scale of the base font size; below that minimum the label truncates rather than wraps. This policy MUST be applied consistently to every button in the app through a shared label component, including the shared `VioletButton`.
